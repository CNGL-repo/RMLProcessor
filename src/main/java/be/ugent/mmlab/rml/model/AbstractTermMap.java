...
public abstract class AbstractTermMap implements TermMap {
	...
	protected AbstractTermMap(Value constantValue, URI dataType,
			...
			String inverseExpression, ReferenceIdentifier referenceValue, FunctionCall functionCall)
	...
	private void checkGlobalConsistency() throws InvalidR2RMLStructureException {
		System.out.println(getTermMapType());
		System.out.println(getTermType());
	...
	public Set<ReferenceIdentifier> getReferencedSelectors() {
		...
		case FUNCTION_CALL_VALUED:
			for(ObjectMap om : functionCall.getParameters()) {
				references.addAll(om.getReferencedSelectors());
			}
			break;
