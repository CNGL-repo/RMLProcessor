...
import be.ugent.mmlab.rml.vocabulary.Vocab.RRFTerm;
...
public abstract class RMLMappingFactory {
	...
	public static RMLMapping extractRMLMapping(String fileToRMLFile)
		...
		// Load Scripts
		try {
			URI p_body = r2rmlMappingGraph.URIref(Vocab.RRF_NAMESPACE + RRFTerm.FUNCTION_BODY);
			URI p_name = r2rmlMappingGraph.URIref(Vocab.RRF_NAMESPACE + RRFTerm.FUNCTION_NAME);
	...
	private static ObjectMap extractObjectMap(SesameDataSet r2rmlMappingGraph,
		...
		FunctionCall functionCall = extractFunctionCallFromTermMap(r2rmlMappingGraph, object, RRFTerm.FUNCTION_CALL, graphMaps, triplesMapResources);
	...
	private static FunctionCall extractFunctionCallFromTermMap(
			SesameDataSet r2rmlMappingGraph, 
			Resource termType,
			RRFTerm term, Set<GraphMap> graphMaps, Map<Resource, TriplesMap> triplesMapResources) 
					throws InvalidR2RMLStructureException {

		URI p = r2rmlMappingGraph.URIref(Vocab.RRF_NAMESPACE + term);
		...
		URI predicate = r2rmlMappingGraph.URIref(Vocab.RRF_NAMESPACE + RRFTerm.FUNCTION);
		...
		URI functionName = r2rmlMappingGraph.URIref(Vocab.RRF_NAMESPACE + RRFTerm.FUNCTION_NAME);
		...
		predicate = r2rmlMappingGraph.URIref(Vocab.RRF_NAMESPACE + RRFTerm.PARAMETER_BINDINGS);
		...
		if(stmts.size() == 0) {
			...
			while(true){
				...
				if(stmts.size() == 1) {
					...
					try {
						ObjectMap objectmap = extractObjectMap(r2rmlMappingGraph, first, graphMaps, triplesMapResources);
						//extractObjectMap(r2rmlMappingGraph, first, graphMaps);
						list.add(objectmap);
					} catch (Exception e) {
						throw new InvalidR2RMLStructureException(
	...
	private static PredicateMap extractPredicateMap(
			SesameDataSet r2rmlMappingGraph, Resource object,
			Set<GraphMap> graphMaps, 
			Map<Resource, TriplesMap> triplesMapResources) throws InvalidR2RMLStructureException,
	R2RMLDataError, InvalidR2RMLSyntaxException {
		...
		PredicateMap result = new StdPredicateMap(null, constantValue,
				stringTemplate, inverseExpression, referenceValue, termType, functionCall);
